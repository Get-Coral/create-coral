...
const ARCHIVE_URL = "https://codeload.github.com/Get-Coral/template/tar.gz/refs/heads/main";
...
${bold}${coral}Coral${reset} ${teal}module scaffolder${reset}`);
  console.log(`${faint}Create a new Jellyfin module from the official Coral template.${reset}\n`);
}
...
async function confirm(rl, question, fallback = true) {
  ...
  const answer = (await rl.question(`${ink}${question}${reset} ${faint}[${label}]${reset}: `)).trim().toLowerCase();
...
async function main() {
  ...
  try {
    ...
    const defaultModuleName = options.moduleName ?? (targetArg === "." ? "coral-module" : path.basename(targetDir));
    ...
    if (install === undefined) {
      install = options.yes ? true : await confirm(rl, "Install dependencies after scaffolding?", true);
    }

    if (!isDirectoryEmpty(targetDir)) {
      const ok = options.yes ? false : await confirm(rl, `Target directory ${path.basename(targetDir)} is not empty. Continue anyway?`, false);
      if (!ok) {
    ...
    try {
      ...
      const extractedRoot = readdirSync(tempDir, { withFileTypes: true })
        .find((entry) => entry.isDirectory() && entry.name.endsWith("-main"))?.name;
      ...
      console.log(`${coral}Scaffolded ${bold}${moduleName}${reset}${coral} in ${targetDir}${reset}`);
      ...
      const displayTargetDir = !relativeTargetDir || relativeTargetDir.startsWith("..")
        ? targetDir
        : relativeTargetDir;
